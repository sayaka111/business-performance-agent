...
class MockLLMClient(LLMClient):
    ...
    def structured_generate(self, *, purpose, payload, schema):
        if purpose == "routing":
            ...
            return {
                "choice": preferred if preferred in candidates else candidates[0],
                "rationale": "offline_mock_candidate_selection",
            }
        if purpose == "report":
            return {
                "selections": [
                    {"claim_id": x["claim_id"], "variant": 0} for x in payload["claims"]
                ]
            }
        if purpose == "intent":
            ...
            if len(dates) != 4 or not re.search(r"gmv", payload["question"], re.I):
                return {
                    "error": "Provide GMV and four ISO dates: current start/end, baseline start/end."
                }
            return {
                "metric_id": payload["allowed_metrics"][0],
                "current_period": {"start": dates[0], "end": dates[1]},
                "baseline_period": {"start": dates[2], "end": dates[3]},
                "filters": {},
            }
