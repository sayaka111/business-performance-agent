class ReportGenerator:
    ...
    def generate(self,result, *, trace=None):
        ...
        self.metadata={'report_renderer':'deterministic_fallback','provider_error_code':None,'retry_count':0}
        ...
        if self.client and findings:
            ...
            try:
                ...
                if any(set(x)!={'claim_id','variant'} or type(x['variant']) is not int or x['variant']!=0 for x in selections): raise ValueError('unapproved wording')
                lines=[by_id[x['claim_id']] for x in selections]
                if getattr(self.client,'provider_name',None)=='gemini':
                    self.metadata['report_renderer']='gemini'
            except Exception as exc:
                lines=[x['claim'] for x in findings]
                code=getattr(exc,'code',None)
                if type(code) is int: self.metadata['provider_error_code']=code
            finally:
                events=[e for e in getattr(self.client,'events',[])[event_start:] if e.get('purpose')=='report']
                self.metadata['retry_count']=max((e.get('attempt',1)-1 for e in events),default=0)
                codes=[e['http_status'] for e in events if type(e.get('http_status')) is int]
                if codes: self.metadata['provider_error_code']=codes[-1]
        if trace is not None: trace.update(self.metadata)
    ...
    def render_markdown(result,lines):
        ...
        for title,items in [('分析发现',lines),('警告',result['warnings']),('限制',result['limitations'])]:
            if items:
