...
def candidate_claims(evidence):
    ...
    for e in evidence:
        payload = e["payload"]
        if e["evidence_level"] not in ("direct", "derived") or payload.get(
            "kind"
        ) not in ("comparison", "contribution"):
            continue
        claims.append(
            dict(
                claim_id="C" + e["evidence_id"][1:],
                kind=payload["kind"],
                fact=payload["fact"],
                scope=payload["scope"],
...
def claim_text(claim):
    ...
    if claim["kind"] == "comparison":
        relative = (
            "undefined"
            if fact["relative_change"] is None
            else f"{fact['relative_change']:.2%}"
        )
    ...
    if not aligned:
        return f"{scope['metric_id']}{qualifier} 无同向贡献项。"
    top = aligned[0]
...
def build_result(definition, state, target, primary, secondary, claims):
    def driver(value):
        return (
            dict(
                metric_id=value["id"],
                effect=value["effect"],
                aligned_share=value["aligned_share"],
            )
            if value
            else dict(metric_id=None, effect=None, aligned_share=None)
        )
    ...
    result = dict(
        ...
        target={
            key: target.get(key, state.target_metric if key == "metric_id" else None)
            for key in (
                "metric_id",
                "current_value",
                "baseline_value",
        ...
        primary_driver=driver(primary),
        secondary_drivers=[driver(x) for x in secondary],
        diagnostic_path=[
            {
                key: step.get(key)
                for key in (
            ...
            for step in state.analysis_path
        ],
        key_findings=[
            dict(
                claim_id=c["claim_id"],
        ...
        evidence=[
            e for e in state.evidence if e["evidence_level"] in ("direct", "derived")
        ],
        warnings=list(dict.fromkeys(state.warnings)),
        limitations=list(dict.fromkeys(state.limitations)),
        failed_branches=state.failed_branches,
...
def validate_result(result, schema):
    ...
    def validate(value, template, path):
        if isinstance(template, dict):
            ...
            if not any(types[t](value) if t in types else value == t for t in options):
                raise ValueError(path)
