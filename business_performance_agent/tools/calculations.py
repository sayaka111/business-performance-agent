...
def closure_check(target, effects, *, tolerance=1e-9):
    ...
    return {'closure_error': error, 'closed': abs(error) <= tolerance * max(1, abs(target))}

def direction_aligned_effect(change, effect):
    return 'aligned_driver' if change*effect > 0 else 'offset' if change*effect < 0 else 'neutral'

def rank_effects(change, effects):
    aligned_total = math.fsum(abs(x['effect']) for x in effects if change*x['effect'] > 0)
    enriched = [dict(x, role=direction_aligned_effect(change,x['effect']), aligned_share=(abs(x['effect'])/aligned_total if aligned_total and change*x['effect']>0 else 0.0)) for x in effects]
    return sorted(enriched, key=lambda x: (x['role'] != 'aligned_driver', -abs(x['effect']), x['id']))

def coverage(effects):
    return min(1.0, math.fsum(x['aligned_share'] for x in effects if x['role'] == 'aligned_driver'))
...
def evaluate_formula(formula, resolve):
    ...
    def walk(node):
        ...
        if isinstance(node, ast.BinOp):
            ...
            if isinstance(node.op, ast.Div): return None if right == 0 else left/right
