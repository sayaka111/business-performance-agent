...
class Executor:
    def __init__(self, knowledge, query, state, trace, policy):
        self.knowledge, self.query, self.state, self.trace, self.policy = (
            knowledge,
            query,
            state,
            trace,
    ...
    def call(self, name, **kwargs):
        ...
        for attempt in range(self.policy["retry_limit"] + 1):
            try:
                ...
                result = ModuleResult(
                    "failed", reason="execution_failure", limitations=[str(exc)]
                )
                retry = attempt < self.policy["retry_limit"]
            except BoundaryError as exc:
                result = ModuleResult(
                    "blocked", reason=exc.reason, limitations=[exc.detail]
                )
                retry = False
                ...
                retry = (
                    result.status == "failed" and attempt < self.policy["retry_limit"]
                )
            self.trace["skills_called"].append(
                {
