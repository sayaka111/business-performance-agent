...
class Router:
    ...
    def rule(self, node, condition):
        return next(r for r in self.rules if r['from']==node and r['condition']==condition)

    def stop(self, state, *, lateral=False):
        if state.depth >= self.policy['max_depth']: return 'max_depth_reached'
        if state.selected_driver and state.selected_driver['aligned_share'] < self.policy['branch_min_contribution']:
    ...
    def select(self, candidates, context, node, trace):
        if len(candidates) == 1:
            decision = {'choice':candidates[0], 'rationale':'unique_valid_candidate','mode':'deterministic'}
        elif not candidates:
            decision = {'choice':'no_valid_choice','rationale':'empty_candidate_set','mode':'deterministic'}
        else:
            decision = self.constrained.choose(candidates,context,node)
        trace['router_decisions'].append(dict(node=node,allowed_candidates=candidates,**decision))
    ...
    def significant(self, contribution):
        ...
        secondary = [x for x in aligned[1:] if x['aligned_share'] >= self.policy['retain_secondary_driver_if_contribution_gte']]
