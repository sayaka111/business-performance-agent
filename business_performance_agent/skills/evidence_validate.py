...
def execute(knowledge, query, *, claim, evidence):
    ...
    if not isinstance(claim, dict) or claim.get("kind") not in (
        ...
        return ModuleResult(
            "blocked",
            result={"claim_status": "unsupported", "evidence_level": "unsupported"},
            reason="evidence_boundary_reached",
        )
    by_id = {e["evidence_id"]: e for e in evidence}
    refs = claim.get("evidence_refs", [])
    valid = bool(refs) and all(
        ref in by_id and by_id[ref]["evidence_level"] in ("direct", "derived")
        for ref in refs
    )
    if valid:
        source = by_id[refs[0]]
        valid = source["payload"].get("fact") == claim.get("fact") and source[
            "payload"
        ].get("scope") == claim.get("scope")
