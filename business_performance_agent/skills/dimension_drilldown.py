    ...
    for segment in sorted(set(current["values"]) | set(baseline["values"])):
        ...
        rows.append(
            dict(
                ...
                current=c,
                baseline=b,
                absolute_change=absolute_change(c, b),
                relative_change=relative_change(c, b),
            )
        )
    return ModuleResult(
        result=dict(
            ...
            contribution_supported=supported,
            segments=rows,
        ...
        if supported
            ...
            "Non-additive metric/dimension: descriptive comparison only; strict contribution prohibited."
        ],
    )
