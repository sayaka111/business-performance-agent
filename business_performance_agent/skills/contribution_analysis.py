...
def execute(
    ...
    *,
    ...
    if dimension_id:
        ...
        if (
            relationship["target_metric"] != target_metric
            or relationship["type"] == "diagnostic"
        ):
            return ModuleResult("blocked", reason="semantic_conflict")
        kind = relationship["type"]
        ids = relationship.get(
    ...
    if len(ids) != len(set(ids)):
        return ModuleResult("blocked", reason="semantic_conflict")
    values = [target_baseline, target_current] + [
        x[k] for x in components for k in ("baseline", "current")
    ]
    if any(x is None for x in values):
        return ModuleResult(
            "blocked",
            reason="data_unavailable",
            limitations=["Undefined component prevents exact contribution."],
        )
    if kind == "exact_multiplicative":
        ...
        effects = symmetric_two_factor_decomposition(
            a["baseline"], a["current"], b["baseline"], b["current"]
        )
        ...
        effects = [
            additive_contribution(x["baseline"], x["current"], c)
            for c, x in zip(coefficients, components)
        ]
    else:
        return ModuleResult("blocked", reason="no_valid_relationship")
    checks = [
        closure_check(target_baseline, [baselines]),
        closure_check(target_current, [currents]),
        closure_check(target_current - target_baseline, effects),
    ]
    if not all(x["closed"] for x in checks):
        return ModuleResult(
            "blocked",
            result={"closure_checks": checks},
            reason="semantic_conflict",
            limitations=[
                "Exact relationship/partition does not close; no approximate attribution emitted."
            ],
        )
    return ModuleResult(
        result=dict(
            target_metric=target_metric,
            target_change=target_current - target_baseline,
            relationship_id=relationship_id,
            dimension_id=dimension_id,
            effects=rank_effects(
                target_current - target_baseline,
                [{"id": key, "effect": value} for key, value in zip(ids, effects)],
            ),
            **checks[-1],
        )
    )
