...
class RuntimeTests(Harness):
    def test_default_end_to_end_with_mock_router(self):
        ...
        self.assertEqual(
            {x["skill"] for x in self.engine.trace["skills_called"]},
            set(
                __import__(
                    "business_performance_agent.runtime.executor", fromlist=["SKILLS"]
                ).SKILLS
            ),
        )
    ...
    def test_customer_structure_context(self):
        ...
        self.assertIn(
            ...
            [x["relationship_id"] for x in result["diagnostic_path"]],
        )
    ...
    def test_transient_retry(self):
        ...
        def flaky(*args):
            count[0] += 1
            if count[0] == 1:
                raise TransientError("temporary data read failure")
            return original(*args)

        with patch.object(self.adapter, "metric", side_effect=flaky):
            result = self.run_agent()
        self.assertEqual(result["workflow_status"], "completed")
    ...
    def test_retry_exhaustion(self):
        with patch.object(
            self.adapter, "metric", side_effect=TransientError("unavailable")
        ):
            result = self.run_agent()
        self.assertEqual(result["workflow_status"], "failed")
        calls = [
            x
            for x in self.engine.trace["skills_called"]
            if x["skill"] == "metric_compare"
        ]
        self.assertEqual(len(calls), 2)

    def test_max_depth_stop(self):
        self.definition["policy"]["diagnosis_policy"]["max_depth"] = (
            1  # test-only in-memory policy fixture
        )
