...
from business_performance_agent.tools.query_tool import (
    MockQueryTool,
...
class InterfaceTests(Harness):
    def test_unique_candidate_never_calls_llm(self):
        ...
        with patch.object(
            client, "structured_generate", side_effect=AssertionError("must not call")
        ):
            router = Router(self.definition, ConstrainedRouter(client))
            self.assertEqual(
                router.select(
                    ["aov_units_price"], {}, "node", {"router_decisions": []}
                ),
                "aov_units_price",
            )
    ...
    def test_router_rejects_invented_choice(self):
        client = MockLLMClient()
        with patch.object(
            client, "structured_generate", return_value={"choice": "invented"}
        ):
            with self.assertRaises(BoundaryError):
                ConstrainedRouter(client).choose(
                    ["channel", "category"], {}, "select_next_dimension"
                )
    ...
    def test_intent_validation(self):
        ...
        with self.assertRaises((ValueError, KeyError)):
            parser.parse("invent a business metric")
    ...
    def test_rendering_cannot_add_causal_story(self):
        ...
        with patch.object(
            client,
            "structured_generate",
            return_value={"text": "Competitor bidding caused decline"},
        ):
    ...
    def test_sql_readonly_policy(self):
        ...
        for sql in [
            "SELECT mock_order_key FROM mock_order_items",
            "WITH x AS (SELECT mock_order_key FROM mock_order_items) SELECT mock_order_key FROM x",
        ]:
            self.assertTrue(validate_read_only_sql(sql, policy))
        for sql in [
            "DELETE FROM mock_order_items",
            "SELECT mock_order_key FROM mock_order_items; DROP TABLE x",
            "WITH x AS (DELETE FROM t) SELECT a FROM x",
            "SELECT secret FROM mock_order_items",
            "SELECT mock_order_key FROM private",
            "SELECT * FROM mock_order_items",
        ]:
            with self.assertRaises(BoundaryError):
                validate_read_only_sql(sql, policy)

    def test_query_row_limit_blocks_without_truncation(self):
        query = MockQueryTool(self.adapter, ReadOnlyPolicy(max_rows=1))
        with self.assertRaises(BoundaryError):
            query.metric("orders", self.raw["current_period"], {})
    ...
    def test_physical_mapping_replacement_preserves_result(self):
        ...
        renamed = {
            value: "replacement_" + str(i)
            for i, value in enumerate(sorted(set(old.values())))
        }
    ...
    def test_timeout_rejects_late_response(self):
        with patch(
            "business_performance_agent.tools.query_tool.time.monotonic",
            side_effect=[0, 100],
        ):
            with self.assertRaises(BoundaryError):
                self.query.metric("orders", self.raw["current_period"], {})

    def test_unsupported_core_evidence_removed(self):
        result = self.run_agent()
        result["key_findings"][0]["evidence_level"] = "unsupported"
        with self.assertRaises(ValueError):
            validate_result(result, self.definition["output_schema"])
    ...
    def test_baseline_zero_is_not_fabricated_anomaly(self):
        for row in self.adapter.rows:
            if row["mock_paid_date"] == "2026-08-25":
                row["mock_goods_amount"] = 0
        result = self.run_agent()
    ...
    def test_failure_after_reliable_root_is_partial(self):
        ...
        def unavailable(metric, *args):
            if metric == "buyers":
                raise TransientError("buyer data temporarily unavailable")
            return original(metric, *args)

        with patch.object(self.adapter, "metric", side_effect=unavailable):
            result = self.run_agent()
