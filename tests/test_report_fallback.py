...
class ReportFallbackTests(unittest.TestCase):
    def setUp(self):
        fixture=json.loads((Path(__file__).parent/'fixtures/gemini_report_failure.json').read_text(encoding='utf-8'))
        self.result={'workflow_status':'completed','stop_reason':'target_coverage_reached',
            'key_findings':[{'claim_id':c['claim_id'],'claim':c['variants'][0]} for c in fixture['payload']['claims']],
            'warnings':['Observed data warning'],'limitations':['Existing evidence boundary']}
        self.reply=json.dumps({'selections':[{'claim_id':c['claim_id'],'variant':0} for c in reversed(self.result['key_findings'])]})
    ...
    def test_transient_failure_uses_markdown_fallback_and_traces_report_only(self):
        for code in (429,503,504):
            with self.subTest(code=code):
                ...
                client.events.append({'purpose':'routing','attempt':9,'status':'failed','http_status':500})
                ...
                self.assertEqual(trace,{'report_renderer':'deterministic_fallback','provider_error_code':code,'retry_count':1})
                self.assertEqual(before,self.result)
                self.assertTrue(report.startswith('# GMV 诊断报告\n\n'))
                for finding in self.result['key_findings']: self.assertIn('- '+finding['claim'],report)
                self.assertIn(self.result['warnings'][0],report)
                self.assertIn(self.result['limitations'][0],report)
                self.assertNotIn('private details',report+json.dumps(trace)+json.dumps(client.events))
                self.assertTrue(all(e['transient_provider_failure'] for e in client.events[1:]))

    def test_recovered_gemini_keeps_validated_gemini_order(self):
        client=GeminiLLMClient(transport=Mock(side_effect=[ProviderFailure(503),self.reply]),sleep=Mock())
        renderer=ReportGenerator(client)
        trace={}
        report=renderer.generate(self.result,trace=trace)
        self.assertEqual(trace,{'report_renderer':'gemini','provider_error_code':503,'retry_count':1})
        claims=self.result['key_findings']
        self.assertLess(report.index(claims[-1]['claim']),report.index(claims[0]['claim']))

    def test_normal_success_and_metadata_do_not_leak_previous_failure(self):
        transport=Mock(side_effect=[ProviderFailure(504),ProviderFailure(504),self.reply])
        ...
        self.assertEqual(trace,{'report_renderer':'gemini','provider_error_code':None,'retry_count':0})
    ...
    def test_empty_claims_do_not_call_provider(self):
        ...
        self.assertEqual(renderer.metadata,{'report_renderer':'deterministic_fallback','provider_error_code':None,'retry_count':0})
    ...
    def test_cli_persists_fallback_metadata_without_changing_structured_result(self):
        ...
        def transport(request,schema):
            if request['purpose']=='report': raise ProviderFailure(503)
            return json.dumps(MockLLMClient().structured_generate(purpose=request['purpose'],payload=request['payload'],schema=schema))
        client=GeminiLLMClient(transport=transport,sleep=Mock())
        with tempfile.TemporaryDirectory() as directory:
            settings=Settings(logs=Path(directory))
            output=io.StringIO()
            with patch('sys.argv',['bpa','--gemini','--input',str(settings.root/'examples/gmv_input.json'),'--json']), \
                 patch('business_performance_agent.app.cli.Settings',return_value=settings), \
                 patch('business_performance_agent.llm.gemini_client.GeminiLLMClient',return_value=client), redirect_stdout(output):
            ...
            self.assertEqual(result['execution_mode']['report_rendering'],'deterministic_fallback')
            self.assertTrue(result['report'].startswith('# GMV 诊断报告'))

    @unittest.skipUnless(importlib.util.find_spec('google'), 'Optional Gemini SDK not installed')
    def test_sdk_and_application_retries_do_not_multiply(self):
        ...
        def handler(request):
            ...
            return httpx.Response(503,json={'error':{'code':503,'status':'UNAVAILABLE','message':'Temporary failure'}})
        ...
        with patch.dict('os.environ',{'GEMINI_API_KEY':'synthetic-test-key'}), patch.object(genai,'Client',side_effect=sdk_factory):
            ...
            try:
                ...
                client.close()
        self.assertEqual(len(requests),2,'SDK must not multiply the two application attempts')
        self.assertEqual(renderer.metadata,{'report_renderer':'deterministic_fallback','provider_error_code':503,'retry_count':1})
