...
class GeminiProviderTests(unittest.TestCase):
    ...
    def test_valid_retry(self):
        value = self.run_router(
            ["not json", json.dumps({"choice": "a", "rationale": "available"})]
        )
    ...
    def test_invalid_candidates_extra_fields_empty_missing(self):
        for reply in [
            ...
            with self.subTest(reply=reply), self.assertRaises(BoundaryError):
                self.run_router([reply, reply])
            self.assertEqual(len(self.calls), 2)
    ...
    def test_provider_error_redacted(self):
        ...
        self.assertNotIn(
            "not-for-log", str(error.exception) + json.dumps(self.client.events)
        )

    def test_unknown_purpose_has_no_request(self):
        client = GeminiLLMClient(
            transport=lambda *_: self.fail("Network request forbidden")
        )
        with self.assertRaises(ValueError):
            client.structured_generate(purpose="sql", payload={}, schema={})
    ...
    def test_http_retry_is_bounded(self):
        ...
        with self.assertRaises(BoundaryError):
            self.run_router([ProviderFailure(), ProviderFailure()])
        self.assertEqual(len(self.calls), 2)
        ProviderFailure.code = 400
        with self.assertRaises(BoundaryError):
            self.run_router([ProviderFailure()])
    ...
    def test_report_cannot_add_duplicate_or_free_form_claim(self):
        for response in [
            ...
            client = GeminiLLMClient(
                transport=lambda *_: json.dumps(response), sleep=lambda _: None
            )
            with self.assertRaises(BoundaryError):
                client.structured_generate(
                    purpose="report",
                    payload={"claims": [{"claim_id": "c1", "variants": ["verified"]}]},
