...
class Harness(unittest.TestCase):
    def setUp(self):
        ...
        self.raw=json.loads((self.settings.root/'examples/gmv_input.json').read_text())

    def run_agent(self,client=None,raw=None):
        self.engine=Runtime(self.knowledge,self.query,self.definition,self.settings,client)
    ...
    def comparison_args(self,metric='orders'):
        return dict(metric_id=metric,current_period=self.raw['current_period'],baseline_period=self.raw['baseline_period'],filters={})
