...
class CLIIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.request = json.loads(
            (self.root / "examples/gmv_input.json").read_text(encoding="utf-8")
        )
    ...
    def invoke(self, *arguments, command=None):
        ...
        process = subprocess.run(
            command or [sys.executable, "-m", "business_performance_agent", *arguments],
            cwd=self.root,
            env=env,
            capture_output=True,
            text=True,
        ...
        self.assertEqual(
            self.before,
            self.source_hashes(),
            "CLI changed business specifications or implementation",
        )
        return process
    ...
    def assert_output(self, process, code=0, llm="mock"):
        ...
        output = json.loads(
            process.stdout
        )  # Fails if JSON stdout is contaminated by prose.
        self.assertEqual(
    ...
    def test_structured_mock_request_runs_original_workflow(self):
        ...
        output, trace = self.assert_output(
            self.invoke("--input", str(request_path), "--mock-llm", "--json")
        )
    ...
    def test_unconfigured_llm_preserves_actual_stop_reason(self):
        output, trace = self.assert_output(
            self.invoke("--input", "examples/gmv_input.json", "--json"),
            llm="unconfigured",
        )
        self.assertEqual(output["result"]["stop_reason"], "evidence_boundary_reached")
        self.assertTrue(
            any(
                d.get("rationale") == "llm_provider_not_configured"
                for d in trace["router_decisions"]
            )
        )
    ...
    def test_unsupported_input_returns_original_blocked_json(self):
        for change in (
            ...
            {"current_period": {"start": "2026-09-01", "end": "2026-09-05"}},
        ):
            with self.subTest(change=change):
                ...
                output, trace = self.assert_output(
                    self.invoke("--input", str(path), "--mock-llm", "--json"), code=1
                )
                self.assertEqual(
                    trace["input"], raw
                )  # No silent field removal or date substitution.
                self.assertEqual(output["result"]["workflow_status"], "blocked")
                self.assertEqual(
                    output["result"]["stop_reason"],
                    "invalid_input" if "intent" in change else "data_quality_boundary",
                )
    ...
    def test_sqlite_metadata_and_windows_passthrough(self):
        database = self.root / "records.db"
        mapping = build_fixture(database, smoke_records(), SMOKE_PERIODS)
        args = [
            "--input",
            "examples/gmv_input.json",
            "--database",
            str(database),
            "--mapping",
            str(mapping),
        ...
        self.assertEqual(output["execution_mode"]["data_backend"], "sqlite")
        self.assertEqual(output["execution_mode"]["data_origin"], "synthetic")
        self.assertFalse(output["execution_mode"]["production_database"])
        self.assertNotIn("real_business_data", output["execution_mode"])
        self.assertEqual(output["result"]["workflow_status"], "completed")
        if os.name == "nt":
            ...
            if shell:
                ...
                wrapped = self.invoke(command=command)
                self.assertEqual(wrapped.returncode, 0, wrapped.stderr)
                self.assertEqual(json.loads(wrapped.stdout)["result"], output["result"])
                invalid = self.invoke(command=command + ["-Model", "offline-probe"])
                self.assertEqual(invalid.returncode, 2)
                self.assertIn("--model requires --gemini", invalid.stderr)
    ...
    def test_utf8_bom_request_preserves_user_filters(self):
        ...
        path.write_text(
            json.dumps(self.request, ensure_ascii=False), encoding="utf-8-sig"
        )
        output, trace = self.assert_output(
            self.invoke("--input", str(path), "--mock-llm", "--json")
        )
