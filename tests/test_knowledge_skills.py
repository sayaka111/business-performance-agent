...
from business_performance_agent.skills import contribution_analysis, dimension_drilldown, evidence_validate, metric_decompose, anomaly_evaluate
...
class KnowledgeSkillTests(Harness):
    def test_knowledge_defensive_copy_and_unchanged_sources(self):
        ...
        metric['formula']='0'
        self.assertEqual(self.knowledge.get_metric('aov')['formula'],'gross_gmv / orders')
    ...
    def test_unsupported_ids_block(self):
        with self.assertRaises(BoundaryError): self.knowledge.get_metric('invented')

    def test_no_relationship_selection_inside_skill(self):
        result=metric_decompose.execute(self.knowledge,self.query,**self.comparison_args())
        self.assertEqual(set(result.result['available_relationships']),{'orders_buyers_frequency','orders_new_returning'})

    def test_nonadditive_dimension_is_descriptive(self):
        for dimension in ('category','product'):
            with self.subTest(dimension=dimension):
                result=dimension_drilldown.execute(self.knowledge,self.query,**self.comparison_args(),dimension_id=dimension)
                self.assertFalse(result.result['contribution_supported'])
                self.assertTrue(result.limitations)
                blocked=contribution_analysis.execute(self.knowledge,self.query,target_metric='orders',target_baseline=100,target_current=75,
                    components=result.result['segments'],dimension_id=dimension)
    ...
    def test_knowledge_contract_and_initial_state(self):
        ...
        self.assertTrue(self.knowledge.get_dimension('channel')['contribution_support']['orders'])
        state=WorkflowState()
    ...
    def test_closure_conflict_blocks(self):
        result=contribution_analysis.execute(self.knowledge,self.query,target_metric='gross_gmv',target_baseline=1000,target_current=800,
            relationship_id='gross_gmv_orders_aov',components=[dict(metric_id='orders',baseline=10,current=8),dict(metric_id='aov',baseline=100,current=90)])
        self.assertEqual(result.reason,'semantic_conflict')

    def test_unknown_preserved_and_closes(self):
        result=dimension_drilldown.execute(self.knowledge,self.query,**self.comparison_args(),dimension_id='channel')
    ...
    def test_threshold_boundaries(self):
        for relative,severity in [(0.1,'warning'),(-0.2,'critical'),(0.099,'normal')]:
            result=anomaly_evaluate.execute(self.knowledge,self.query,metric_compare_result={'metric_id':'gross_gmv','relative_change':relative},anomaly_policy=self.definition['policy']['anomaly_policy'])
            self.assertEqual(result.result['severity'],severity)

    def test_unsupported_causal_claim_rejected(self):
        self.assertEqual(evidence_validate.execute(self.knowledge,self.query,claim='Competitor bidding caused decline',evidence=[]).status,'blocked')
